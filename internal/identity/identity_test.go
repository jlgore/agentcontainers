...
func TestVCRoundTripAndExpiry(t *testing.T) {
	...
	// Tampered signature fails. Flip a base64url character in the MIDDLE of the
	// signature (well before the final char, whose trailing bits are padding
	// the decoder ignores) to a guaranteed-different one, so the mutation always
	// changes the decoded signature bytes. Earlier versions mutated the last
	// char / appended a fixed suffix, which was a decode no-op for some
	// signatures and made this test intermittently pass.
	i := len(tok) - 40 // inside the 86-char ed25519 signature, away from padding
	repl := byte('A')
	if tok[i] == 'A' {
		repl = 'B'
	}
	bad := tok[:i] + string(repl) + tok[i+1:]
